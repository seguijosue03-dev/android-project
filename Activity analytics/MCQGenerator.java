...
public class MCQGenerator {
    ...
    // specific set to track question text to prevent duplicates
    private final Set<String> generatedQuestionTexts = new HashSet<>();
    ...
    public List<MCQQuestion> generateQuestions(PPTContent pptContent, int requestedCount) {
        generatedQuestionTexts.clear(); // Reset duplicates
        ...
        int slideIndex = 0;
        int attempts = 0;
        // ✅ FIX 1: Increased max attempts significantly to prevent early quitting
        int maxAttempts = requestedCount * 15;
        ...
        while (result.size() < requestedCount && attempts < maxAttempts) {
            ...
            if (q != null && !generatedQuestionTexts.contains(q.getQuestion())) {
                result.add(q);
                generatedQuestionTexts.add(q.getQuestion());
            }
            ...
            if (slideIndex >= shuffledSlides.size()) {
                slideIndex = 0;
                Collections.shuffle(shuffledSlides, random); // Reshuffle to find new angles
            }
        }
    ...
    private MCQQuestion generateFromSlide(PPTContent.SlideContent slide, List<PPTContent.SlideContent> allSlides) {
        ...
        // ✅ FIX 3: Loop through ALL types if the random choice fails
        // This ensures if a slide can ONLY produce a "Fill in Blank", we find it.
        int startType = random.nextInt(5);
        for (int i = 0; i < 5; i++) {
            int currentType = (startType + i) % 5; // Cycles 0,1,2,3,4,0...

            MCQQuestion q = null;
            switch (currentType) {
                case 0: q = definitionMCQ(topic, sentences, allSlides); break;
                case 1: q = trueStatementMCQ(topic, sentences); break;
                case 2: q = notTypeMCQ(topic, slide, allSlides); break;
                case 3: q = fillBlankMCQ(topic, sentences); break;
                case 4: q = fullFormMCQ(topic, sentences); break;
            }

            if (q != null) return q;
        }

        // ✅ FIX 4: Emergency Backup
        // If all logic fails, make a very simple valid/invalid question using any sentence
        return trueStatementMCQ(topic, sentences);
    }
    ...
    private MCQQuestion definitionMCQ(String topic, List<String> sentences, List<PPTContent.SlideContent> allSlides) {
        ...
        for (PPTContent.SlideContent other : allSlides) {
            ...
            for (String s : other.getContentPoints()) {
                List<String> cleanedList = cleanSentences(Collections.singletonList(s));
                if (!cleanedList.isEmpty()) {
                    String cleaned = cleanedList.get(0);
                    if (cleaned.matches("(?i).*(is a|is the|refers to|means|defined as).*")) {
    ...
    private MCQQuestion trueStatementMCQ(String topic, List<String> sentences) {
        // Relaxed requirement: Need just 1 good sentence
        if (sentences.isEmpty()) return null;

        String correctSentence = sentences.get(random.nextInt(sentences.size()));

        String question = "Which statement is correct regarding " + topic + "?";
        List<String> options = new ArrayList<>();
        options.add(cleanAnswer(correctSentence));

        // Generate 3 wrong options
        while(options.size() < 4) {
            String wrong = makeSimilarButWrong(correctSentence, topic);
            if(!options.contains(wrong)) {
                options.add(wrong);
            } else {
                // If we run out of ideas, generic wrong answers
                String genericWrong = generatePlausibleWrongAnswer(topic);
                if(!options.contains(genericWrong)) options.add(genericWrong);
                else break;
            }
        }

        // Fill remaining with None of above if needed
        while(options.size() < 4) {
            if(!options.contains("None of the above")) options.add("None of the above");
            else options.add("All of the above"); // Fallback
        }

        Collections.shuffle(options, random);
        int correctIndex = options.indexOf(cleanAnswer(correctSentence));
    ...
    private MCQQuestion notTypeMCQ(String topic, PPTContent.SlideContent currentSlide, List<PPTContent.SlideContent> allSlides) {
        ...
        if (sentences.size() < 2) return null; // Relaxed from 3 to 2
        ...
        // We need 1 wrong answer (which is effectively from another topic)
        ...
        // If we couldn't find a distractor from slides, generate a fake one
        if (differentOption == null) {
            differentOption = generatePlausibleWrongAnswer("Random Topic");
            options.add(differentOption);
        }

        // Fill up to 4
        while (options.size() < 4) {
            options.add(generatePlausibleWrongAnswer(topic));
        }

        Collections.shuffle(options, random);
        // The "Correct Answer" in a NOT question is the one that DOESN'T belong
    ...
    private MCQQuestion fillBlankMCQ(String topic, List<String> sentences) {
        ...
        for (String s : sentences) {
            // Relaxed regex: just looks for is/are
            if (s.matches("(?i).*\\b(is|are)\\b.*")) {
                targetSentence = s;
                break;
            }
        }
    ...
    private MCQQuestion fullFormMCQ(String topic, List<String> sentences) {
        ...
        while (options.size() < 4 && attempts < 50) {
            ...
            if (deceptive.equalsIgnoreCase(acronym)) { attempts++; continue; }
    ...
    private String generateDeceptiveFullForm(String correctFullForm) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            if (cleanWord.isEmpty()) { deceptive.append(word); continue; }
            ...
            if (shouldChange) {
                ...
                if (candidates != null && !candidates.isEmpty()) {
                    ...
                    if (replacement.equalsIgnoreCase(cleanWord)) replacement = candidates.get(random.nextInt(candidates.size()));
                    deceptive.append(replacement);
                } else { deceptive.append(word); }
            } else { deceptive.append(word); }
    ...
    // ✅ FIX 5: Relaxed Filters heavily
    private List<String> cleanSentences(List<String> raw) {
        List<String> cleaned = new ArrayList<>();
        for (String s : raw) {
            if (s.matches(".*[<>{}\\[\\]].*") || s.trim().isEmpty()) continue;

            // RELAXED: Allow shorter sentences (down to 10 chars)
            if (s.length() < 10) continue;

            s = s.replaceAll("\\s+", " ").trim();
            s = s.replaceAll("^[*\\-•]+\\s*", "");

            // RELAXED: Allow sentences with just 3 words (e.g., "Java is fast")
